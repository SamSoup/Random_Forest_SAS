...
fst_mse = function(idx) {
  if(length(idx)>0){
          mid1 = order(s_fst[idx],decreasing=TRUE,na.last=TRUE)[1]
          fst_max = s_fst[idx][mid1]
          mean_err = c()
          for(maxid in which(s_fst[idx] == fst_max)){
                ...
                fst2 = s_fst[idx2]
                mean_err = c(mean_err,mean((fst2 - (tail_fst + (fst_max-tail_fst)*(1-pos2/wnd_size)))^2,na.rm=TRUE))
          }
        return(data.frame(fst_max=fst_max,mean_err=min(mean_err)))
  } else {
